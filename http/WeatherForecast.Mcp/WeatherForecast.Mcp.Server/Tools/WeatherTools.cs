...
namespace WeatherForecast.Mcp.Server.Tools
{
    [McpServerToolType]
    public sealed class WeatherTools
    {
        ...
        [McpServerTool, Description("Get a random weather forecast.")]
        public async Task<string> GetRandomWeatherForecastAsync()
        {
            var client = _httpClientFactory.CreateClient("RandomWeatherForecastApi");
            var forecast = await client.GetFromJsonAsync<WeatherForecast[]>("weatherforecast")
                ?? throw new McpException("No JSON returned from weatherforecast endpoint");
        ...
        /// </summary>
        /// <param name="state">The two-letter abbreviation of the U.S. state for which to retrieve weather alerts. For example, "NY" for
        /// New York. Must be a valid state code.</param>
        /// <returns>A string containing the details of all active weather alerts for the specified state, or a message
        /// indicating that there are no active alerts.</returns>
        /// <exception cref="McpException">Thrown if the weather alerts endpoint does not return valid JSON data.</exception>
